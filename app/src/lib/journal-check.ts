...
export async function checkAgainstJournal(opts: {
  ...
  const judged = await judgeWithLlm(opts.flaggedText, opts.claimText, abstract);
  if (judged) return { verdict: judged.verdict, note: judged.reason, pmid: ref.pmid };

  // No API key (or model unavailable): still valuable — surface the abstract
