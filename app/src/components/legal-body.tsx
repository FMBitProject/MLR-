...
function withPlaceholders(text: string) {
  return text.split(PLACEHOLDER).map((part, i) =>
    PLACEHOLDER.test(part) ? (
      <mark
        key={i}
        className="rounded bg-amber-400/20 px-1.5 py-0.5 font-medium text-amber-100 ring-1 ring-inset ring-amber-400/40"
      >
        {part}
      </mark>
    ) : (
    ...
    <div className="space-y-9">
      {sections.map((s) => (
        <section key={s.heading}>
          ...
          {s.list ? (
            <ul className="mt-3 space-y-2 border-l-2 border-white/15 pl-4">
              {s.list.map((li, i) => (
                <li key={i} className="text-[14px] leading-relaxed text-slate-300">
                  {withPlaceholders(li)}
                </li>
              ))}
            </ul>
